...
def random_jpeg_quality(image,
  ...
  with tf.compat.v1.name_scope('RandomJpegQuality', values=[image]):
    ...
    image = tf.cond(pred=do_encoding_random, true_fn=_adjust_jpeg_quality,
                    false_fn=lambda: tf.cast(image, tf.float32))
...
def random_downscale_to_target_pixels(image,
  ...
  with tf.compat.v1.name_scope('RandomDownscaleToTargetPixels', values=[image]):
    generator_fn = functools.partial(tf.random.uniform, [], seed=seed)
    ...
    if masks is not None:
      ...
      return tf.cond(pred=should_apply_resize, true_fn=resize_fn,
                     false_fn=lambda: (tf.cast(image, dtype=tf.float32), masks))
    else:
      resize_fn = lambda: _resize_image_to_target(target_height, target_width)
      return tf.cond(pred=should_apply_resize, true_fn=resize_fn,
                     false_fn=lambda: (tf.cast(image, dtype=tf.float32),))
...
def random_patch_gaussian(image,
  ...
  def _apply_patch_gaussian(image):
    ...
    patched_image = tf.compat.v1.where(patch_mask, image_plus_gaussian, scaled_image)
  ...
  with tf.compat.v1.name_scope('RandomPatchGaussian', values=[image]):
    ...
    patch_gaussian_random = get_or_create_rand_vars_fn(
        functools.partial(tf.random.uniform, [], seed=seed))
    do_patch_gaussian = tf.greater_equal(patch_gaussian_random, random_coef)
    image = tf.cond(pred=do_patch_gaussian,
                    true_fn=lambda: _apply_patch_gaussian(image),
                    false_fn=lambda: image)
  return image
