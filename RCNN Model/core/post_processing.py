...
def class_agnostic_non_max_suppression(boxes,
  ...
  if q > 1:
    ...
    boxes = tf.compat.v1.batch_gather(boxes, class_ids)
    if masks is not None:
      masks = tf.compat.v1.batch_gather(masks, class_ids)
    if boundaries is not None:
      boundaries = tf.compat.v1.batch_gather(boundaries, class_ids)
