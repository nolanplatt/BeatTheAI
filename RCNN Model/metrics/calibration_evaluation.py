...
class CalibrationDetectionEvaluator(
  ...
  def match_single_image_info(self, image_info):
    ...
    one_hot_depth = tf.cast(tf.add(tf.reduce_max(input_tensor=groundtruth_classes), 1),
                            dtype=tf.int32)
    ...
    cls_targets = tf.argmax(input=one_hot_cls_targets, axis=1)
  ...
  def get_estimator_eval_metric_ops(self, eval_dict):
    ...
    image_info = [tf.boolean_mask(tensor=tensor, mask=is_annotated_batched) for tensor in
    ...
    y_pred = tf.squeeze(tf.boolean_mask(tensor=detection_scores, mask=is_annotated_batched))
