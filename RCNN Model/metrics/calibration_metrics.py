...
def _safe_div(numerator, denominator):
  ...
  return tf.compat.v1.where(condition, t, zero)
...
def _ece_from_bins(bin_counts, bin_true_sum, bin_preds_sum, name):
  ...
  bin_weights = _safe_div(bin_counts, tf.reduce_sum(input_tensor=bin_counts))
  return tf.reduce_sum(input_tensor=abs_bin_errors * bin_weights, name=name)
...
def expected_calibration_error(y_true, y_pred, nbins=20):
  ...
  with tf.control_dependencies([
      tf.compat.v1.assert_greater_equal(y_pred, 0.0),
      tf.compat.v1.assert_less_equal(y_pred, 1.0),
  ]):
    bin_ids = tf.histogram_fixed_width_bins(y_pred, [0.0, 1.0], nbins=nbins)

  with tf.control_dependencies([bin_ids]):
    update_bin_counts_op = tf.compat.v1.assign_add(
        bin_counts, tf.cast(tf.math.bincount(bin_ids, minlength=nbins),
                            dtype=tf.float32))
    update_bin_true_sum_op = tf.compat.v1.assign_add(
        bin_true_sum,
        tf.cast(tf.math.bincount(bin_ids, weights=y_true, minlength=nbins),
                dtype=tf.float32))
    update_bin_preds_sum_op = tf.compat.v1.assign_add(
        bin_preds_sum,
        tf.cast(tf.math.bincount(bin_ids, weights=y_pred, minlength=nbins),
                dtype=tf.float32))
