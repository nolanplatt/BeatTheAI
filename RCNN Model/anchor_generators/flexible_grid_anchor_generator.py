  ...
  def _generate(self, feature_map_shape_list, im_height=1, im_width=1):
    ...
    for (feat_shape, base_sizes, aspect_ratios, anchor_stride, anchor_offset
      ...
      anchor_grid = grid_anchor_generator.tile_anchors(
          feat_shape[0],
          feat_shape[1],
          tf.cast(tf.convert_to_tensor(value=base_sizes), dtype=tf.float32),
          tf.cast(tf.convert_to_tensor(value=aspect_ratios), dtype=tf.float32),
          tf.constant([1.0, 1.0]),
          tf.cast(tf.convert_to_tensor(value=anchor_stride), dtype=tf.float32),
          tf.cast(tf.convert_to_tensor(value=anchor_offset), dtype=tf.float32))
