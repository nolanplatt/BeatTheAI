...
def build_graph(pipeline_config,
  ...
  def tpu_subgraph_predict_fn(preprocessed_inputs, true_image_shapes):
    ...
    return (
        ...
        tf.transpose(a=prediction_dict[RPN_BOX_ENCODINGS], perm=[2, 0, 1]),
        # [batch, anchor, depth] -> [depth, batch, anchor]
        tf.transpose(
            a=prediction_dict[RPN_OBJECTNESS_PREDICTIONS_WITH_BACKGROUND],
            perm=[2, 0, 1]),
        # [anchors, depth]
  ...
  def tpu_subgraph_predict():
    if use_bfloat16:
      with tf.compat.v1.tpu.bfloat16_scope():
        return tf.compat.v1.tpu.rewrite(tpu_subgraph_predict_fn,
  ...
  prediction_dict = {
      RPN_BOX_ENCODINGS:
          tf.transpose(a=rpn_box_encodings, perm=[1, 2, 0]),
      RPN_OBJECTNESS_PREDICTIONS_WITH_BACKGROUND:
          tf.transpose(
              a=rpn_objectness_predictions_with_background, perm=[1, 2, 0]),
      ANCHORS:
          tf.transpose(a=anchors, perm=[1, 0]),
