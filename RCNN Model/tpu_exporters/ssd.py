...
def recover_shape(preprocessed_inputs, prediction_outputs, shapes_info):
  ...
  preprocessed_inputs = tf.transpose(a=preprocessed_inputs, perm=[0, 2, 3, 1])

  box_encodings = tf.transpose(a=prediction_outputs[0], perm=[1, 2, 0])
...
def build_graph(pipeline_config,
  ...
  preprocessed_inputs = tf.transpose(a=preprocessed_inputs, perm=[0, 3, 1, 2])
  if use_bfloat16:
  ...
  def predict_tpu_subgraph(preprocessed_inputs, true_image_shapes):
    ...
    return [
        tf.transpose(a=prediction_dict[BOX_ENCODINGS], perm=[2, 0, 1]),
        tf.transpose(
            a=prediction_dict[CLASS_PREDICTIONS_WITH_BACKGROUND], perm=[2, 0, 1]),
        tf.transpose(a=prediction_dict[ANCHORS], perm=[1, 0]),
    ]

  @function.Defun(capture_resource_var_by_value=False)
  def predict_tpu():
    return tf.compat.v1.tpu.rewrite(predict_tpu_subgraph,
