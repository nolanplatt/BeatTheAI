  ...
  def test_use_bounded_activations_add_operations(
    ...
    with tf_graph.as_default():
      image_features = [('block2',
                         tf.random.uniform([4, 8, 8, 256], dtype=tf.float32)),
                        ('block3',
                         tf.random.uniform([4, 4, 4, 256], dtype=tf.float32)),
                        ('block4',
                         tf.random.uniform([4, 2, 2, 256], dtype=tf.float32)),
                        ('block5',
                         tf.random.uniform([4, 1, 1, 256], dtype=tf.float32))]
      feature_map_generator = self._build_feature_map_generator(
          image_features=image_features,
          depth=128,
          use_keras=use_keras,
...
class PoolingPyramidFeatureMapGeneratorTest(tf.test.TestCase):
  ...
  def test_get_expected_feature_map_shapes(self, replace_pool_with_conv):
    image_features = {
        'image_features': tf.random.uniform([4, 19, 19, 1024])
    }
